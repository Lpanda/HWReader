...
NSDictionary *DictionaryForNode(xmlNodePtr currentNode, NSMutableDictionary *parentResult,BOOL parentContent,int depth)
{
...
//                NSLog(@"Attribute Name Set: %@",attributeName);
          ...
          if (attribute->children)
            {
              NSDictionary *childDictionary = DictionaryForNode(attribute->children, attributeDictionary,true,++depth);
              if (childDictionary)
                {
  ...
  if (childNode)
    {
      ...
      while (childNode)
        {
          NSDictionary *childDictionary = DictionaryForNode(childNode, resultForNode,false, ++depth);
          if (childDictionary)
            {
...
NSArray *PerformXPathQuery(xmlDocPtr doc, NSString *query)
{
  ...
  for (NSInteger i = 0; i < nodes->nodeNr; i++)
    {
      NSDictionary *nodeDictionary = DictionaryForNode(nodes->nodeTab[i], nil,false,0);
      if (nodeDictionary)
        {
